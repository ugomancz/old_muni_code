...
void PIT_SECOND_INTERRUPT(void) {
  uint32_t intStatus;
...
int main(void)
{
    ...
    while (1)
    {
        ...
        if (send_adc_value) {
        	adc_value = ADC16_GetChannelConversionValue(ADC1_PERIPHERAL, 0);
        	sprintf(message, "%d value=%d", configIP_ADDR3, to_percent(adc_value));
        	udp_send_data(fsl_netif0_gw,DESTINATION_PORT,message);
        	send_adc_value = false;
        }
    }
}
#endif
