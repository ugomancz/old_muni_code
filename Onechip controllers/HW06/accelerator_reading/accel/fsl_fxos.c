...
status_t FXOS_Init(fxos_handle_t *fxos_handle, fxos_config_t *config)
{
    ...
    // orientation detection enable
    if (FXOS_WriteReg(fxos_handle, PL_CFG_REG, (PL_EN_MASK)) != kStatus_Success)
    {
    	return kStatus_Fail;
    }

    // orientation detection - enable interrupts
    if (FXOS_WriteReg(fxos_handle, CTRL_REG4, (INT_EN_LNDPRT_MASK)) != kStatus_Success)
       {
       	return kStatus_Fail;
       }

    // orientation detection - enable interrupts
       if (FXOS_WriteReg(fxos_handle, CTRL_REG3, (WAKE_LNDPRT_MASK)) != kStatus_Success)
          {
          	return kStatus_Fail;
          }
    ...
    if (FXOS_WriteReg(fxos_handle, XYZ_DATA_CFG_REG, FULL_SCALE_2G) != kStatus_Success)
    {
        return kStatus_Fail;
    }
#ifdef EN_INTERRUPTS
