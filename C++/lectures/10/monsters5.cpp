// Using the visitor pattern to emulate double dispatch
...
template <typename W>
struct WeaponImpl : Weapon {
	void apply_damage(Monster& monster) override {
		monster.receive_attack(static_cast<W&>(*this));
	}
};


struct Axe : WeaponImpl<Axe> {
	std::string description() const override {
		return "an axe";
	}
};

struct Nuke : WeaponImpl<Nuke> {
	std::string description() const override {
