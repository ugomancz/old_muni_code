...
int main()
{
    ...
    try { m.fetch( t_2 ); assert( false ); } catch ( invalid ) {};
    try { m.make(); assert( false ); } catch ( busy ) {};

    token t_3 = std::move( t_1 );
    try { m.fetch( t_1 ); assert( false ); } catch ( invalid ) {};

    t_2 = std::move( t_3 );
    try { m.fetch( t_3 ); assert( false ); } catch ( invalid ) {};
