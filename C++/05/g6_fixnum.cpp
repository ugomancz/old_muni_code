...
int main()
{
    ...
    try { fixnum( "x" );   assert( false ); } catch ( bad_format ) {}
    try { fixnum( "x.1" ); assert( false ); } catch ( bad_format ) {}
    try { fixnum( "1.." ); assert( false ); } catch ( bad_format ) {}
    try { fixnum( "1.x" ); assert( false ); } catch ( bad_format ) {}
    try { fixnum( "1,1" ); assert( false ); } catch ( bad_format ) {}
}
