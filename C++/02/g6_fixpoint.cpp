 ...
 * that ‹f› is a non-decreasing function (and hence, since there are
 * only finitely many ‹int› values, that it has at least one fixed
 * point). Find the greatest fixed point of ‹f›. */

auto fixpoint = [](auto &f) {
    int index = INT_MAX;
    while (index >= 0) {
        if (f(index) == index) {
            break;
