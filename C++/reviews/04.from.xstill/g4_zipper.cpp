...
/** Tohle je výrazně komplikovanější a výrazně méně efektivní, než zamýšlené
 ** řešení, je to v podstatě linked-list uvnitř vektoru, kdybyste se pokusil
 ** implementovat erase, zjistíte, že nemáte jak smazat to uvolněné místo.
 **
 ** Jde to vyřešit elegantně a bez použití pomocné datové struktury (jakou je
 ** vaše ‹node›) či bez explicitního uchovávání informací o následnících.
 ** Zkuste se ještě zamyslet, co by pro to bylo potřeba. **/
class zipper {
private:
