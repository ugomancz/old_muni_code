...
bool natural::operator==(const natural &other) const {
    /** duplikace funkčnosti standardní knihovny, vector má rovnost **/
...
bool natural::operator<(const natural &other) const {
    /** použití výjimek pro řešení běžného control-flow, výjimky slouží jen k řešení
     ** výjimečných situací **/
    try {
        /** tip:
...
natural &natural::operator+=(const natural &other) {
    /** pokud byste potřeboval číslo měnit, dávalo by větší smysl brát ho hodnotou
     ** (vyhnete se pak zbytečné kopii pokud je argument rvalue), zde by bylo
     ** však ještě lepší se měnění vyhnout úplně, protože fakticky není nutné a
     ** jen zhoršuje paměťovou náročnost operace **/
    auto copy = other;
    uint8_t carried = 0;
    if (copy == natural(0)) {
        return *this;
    ...
    /** při sčítání přirozených čísel je vhodné mít algoritmus napsaný tak, aby
     ** ta 0 ani nevznikla **/
    strip_zeros();
    return *this;
}
...
/** tato funkce nijak netěží z toho, že je this modifikovatelný… dávalo by tedy
 ** větší smysl implementovat ‹*› beroucí obě čísla const-ref a z něj pak
 ** odvodit ‹*=› **/
natural natural::operator*=(const natural &other) {
    if (other == natural(0)) {
        digits.clear();
...
void natural::append_digits_reversed(natural mod) {
    /** existují reverse iterátory, takže není třeba otáčet (a kopírovat),
     ** vkládat rozsah je vhodnější pomocí ‹.insert(digits.end(), …)›, protože
     ** to dokáže udělat jen jeden resize
     **/
...
/** zbytečná kopie, bylo by lepší volat div_mod **/
natural operator/(natural left, const natural &right) {
    left /= right;
    return left;
}
...
std::pair<natural, natural> small_div_mod(natural dividend, const natural &divisor) {
    ...
    while (true) {
        /** opět výjimky použité pro běžné control-flow **/
        try {
            dividend -= divisor;
            mod_result -= divisor;
