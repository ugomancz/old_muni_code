...
fraction eval_continued(const std::vector<int> &coeff) {
    /** zbytečná konverze na menší typ – rozbilo by se to pro hodně velký vektor
     **/
    int index = static_cast<int>(coeff.size() - 1);
    int r, q = 1;
    int p = coeff.at(index);
    /** chtělo by to for cyklus **/
    while (index > 0) {
        r = p * coeff.at(index - 1) + q;
        q = p;
        p = r;
        index--;
    }
