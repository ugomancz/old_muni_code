/** Poměrně rozumné řešení, škoda duplikace ohledně kontroly součtů – je celkem
 ** malá, takže to bude zatím @C **/
...
private:
    ...
    /** není třeba ukládat, velikost lze v konstantním čase zjistit z ‹square›
     **/
    const int size;

    bool solved();

    /** bývá dobrý nápad mít predikáty v pozitivním významu a negativní získávat
     ** negací (‹!used(x)› je krásně čitelné, ‹!unsused(x)› je horší). **/
    bool unused(int value) const;

    int sum_column(int col);

    int sum_row(int row);

    bool empty_cell(int row, int col) const;
