...
public:
    operation(unsigned int table, const node &left, const node &right) : _table(table), _left(left), _right(right) {}

    bool eval(const input &in) const override {
        /** tohle šlo snadno napočítat aritmetickými a bitovými operacemi +
         ** celkem problém je, že opakovaně vyhodnocujete podstromy, to výrazně
         ** kazí složitost **/
        if (_left.eval(in) && _right.eval(in)) {
            return _table & 0b1000;
        } else if (_left.eval(in) && !_right.eval(in)) {
            return _table & 0b0100;
        } else if (!_left.eval(in) && _right.eval(in)) {
            return _table & 0b0010;
        } else { // !_left.eval(in) && !_right.eval(in)
            return _table & 0b0001;
        }
    }
}; /* ref: 16 lines */
