...
public:
    ...
    /** to není dobrý nápad… zavře se to pak dvakrát **/
    fd(const fd &other) = default;

    /** to není dobrý nápad… zavře se to pak dvakrát **/
    fd &operator=(const fd &) = default;

    /** toto je fajn, chválím exchange **/
    ...
    static fd dup(const int other_descriptor) noexcept {
        /** bylo by lepší mít odpovídající konstruktor **/
        fd new_fd{};
    ...
    std::string read(int length) const {
        ...
        /** lepší by bylo číst rovnou do stringu (‹.resize()› + ‹.data()›) **/
        char buf[length];
