...
reg parse_reg_or_immediate(const std::string &in) {
    /** toto není výjimečná situace, neměla by se na to používat výjimka **/
    try {
        return parse_reg(in);
    } catch (no_parse &ex) {}
    size_t pos = 0;
...
instruction parse_line(const std::string &line) {
    ...
    /** naprosto zbytečný catch, vypropaguje se samo **/
    } catch (no_parse &ex) {
        throw;
    }
...
std::vector<instruction> parse(const std::string &input) {
    ...
    while (std::getline(is, line)) {
        ...
        /** zbytečný catch, vypropaguje se samo a číslo řádku si můžete do
         ** funkce poslat, bylo by to lepší než modifikovat výjimku po cestě **/
        } catch (no_parse &ex) {
