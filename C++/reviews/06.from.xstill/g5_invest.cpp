...
class account {
private:
    unsigned int _balance = 0;
public:
    void deposit(const unsigned int sum) noexcept {
        _balance += sum;
    }

    void withdraw(const unsigned int sum) {
        if (sum > _balance) {
    ...
    unsigned int balance() const noexcept {
        return _balance;
    }
}; /* reference implementation: 13 lines */
...
private:
    ...
    int _sum, _rate;
public:
    investment(account &acc, int sum, int rate) : _acc(&acc), _sum(sum), _rate(rate) {
            acc.withdraw(sum);
    }

    investment(const investment &other) = default;
    /** pozor na defaultní copy/move konstruktor/assignment, tu moc nedávají
     ** smysl (rule of 5) **/

    void next_year() noexcept {
        _acc->deposit(_sum * _rate / 100);
    }
