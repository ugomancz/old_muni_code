...
auto mode(const container_t &container) {
    ...
    /** tohle je celkem neefektivní… bylo by lepší proiterovat occurrences a
     ** najít všechny prvky, jejichž second == max_value **/
...
auto median(container_t container) {
    using value_t = typename container_t::value_type;
    /** efektivnější inicializace by byla pomocí konstruktoru vector s
     ** iterátory. Nevýhoda tohoto přístupu je, že vyžaduje aby value_t bylo
     ** default-constructible. Po inicializaci bych použil std::sort. **/
